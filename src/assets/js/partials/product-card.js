...
class ProductCard extends HTMLElement {
  ...
  initCircleBar() {
    ...
    const total = this.product.quantity > 100 ? this.product.quantity * 2 : 100;
    const roundPercent = (qty / total) * 100;
    const bar = this.querySelector('.s-product-card-content-pie-svg-bar');
    const strokeDashOffsetValue = 100 - roundPercent;
  ...
  render() {
    ...
    this.innerHTML = `
        <div class="${!this.fullImage ? 's-product-card-image' : 's-product-card-image-full'}">
          <a href="${this.product?.url}">
            <img class="s-product-card-image-${salla.url.is_placeholder(this.product?.image?.url) ? 'contain' : this.fitImageHeight ? this.fitImageHeight : 'cover'} lazy"
        ...
        <div class="s-product-card-content">
          ...
          <div class="s-product-card-content-main ${this.isSpecial ? 's-product-card-content-extra-padding' : ''}">
            ...
            ${this.product?.subtitle && !this.minimal ? `<p class="s-product-card-content-subtitle">${this.product?.subtitle}</p>` : ``}
          </div>
          ...
          ${this.isSpecial && this.product.discount_ends ? `
            <salla-count-down 
              date="${this.formatDate(this.product.discount_ends)}" 
              end-of-day="true" 
              boxed="true" 
              labeled="true" 
            />` : ``}
    ...
    this.querySelectorAll('[name="donating_amount"]').forEach((element) => {
      element.addEventListener('input', (e) => {
        const donatingAmount = e.target.value;
        const addButton = e.target.closest(".s-product-card-content").querySelector("salla-add-product-button");
